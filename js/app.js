...
$(() => {
  ...
  const checkCol = () => {
    const colClass = $(event.currentTarget).attr('class').match(/left|middle|right/)[0];
    const $currentCol = $(`.${colClass}`);
    const colMoves = getLineText($currentCol);
    checkWinner(colMoves);
  };
  const checkSlash = () => {
    const slashClass = String($(event.currentTarget).attr('class').match(/(?<!back)slash/));
    if (slashClass == 'slash') {
  ...
  const checkBackslash = () => {
    const backslashClass = String($(event.currentTarget).attr('class').match(/backslash/));
